...
NA_LOC(CPRGBColorSpaceTitle, "RGB カラースペース");
...
NA_LOC(CP3DView, "3D ビュー");
NA_LOC(CPCoordinates, "座標");
NA_LOC(CPSteps, "インクリメント");
NA_LOC(CPRotation, "回転");
NA_LOC(CPStop, "止まる");
NA_LOC(CPCartesian, "%s 直交座標系"); // %s is a colorspace name
NA_LOC(CPPointsOpacity, "ポイント 不透明");
NA_LOC(CPGridOpacity, "グリッド 不透明");
NA_LOC(CPGridTint, "グリッド 着色");
NA_LOC(CPBodyTint, "音量 着色");
NA_LOC(CPSolid, "個体 音量");
NA_LOC(CPAxis, "車軸");
